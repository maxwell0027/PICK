...
parser.add_argument('--root_path', type=str, default='/data/userdisk1/qjzeng/semi_seg/BCP-main/code/', help='Name of Dataset')
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, (sampled_batch, mim_mask) in enumerate(trainloader):
            ...
            main_ps_lab   = main_prob[num_cutmix:,1,:,:,:] > 0.5
            ...
            loss_mim = (loss_mim * mask_region).sum() / (mask_region.sum() + 1e-5)
            ...
            if iter_num % 200 == 0:
                model.eval()
                val_dice, maxdice1, max_flag = test(model, model, test_loader, maxdice1, stride_xy=64, stride_z=16)
...
@torch.no_grad()
def test(net, ema_net, val_loader, maxdice=0, stride_xy=16, stride_z=4):
    metrics = test_calculate_metric(net, ema_net, val_loader.dataset, stride_xy=stride_xy, stride_z=stride_z)
    val_dice = metrics[0]
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    #load_net(model, '/data/userdisk1/qjzeng/semi_seg/IJCAI24/weight/Lung_IJCAI_15_labeled/pre_train/VNet_best_model.pth')
    ...
    for epoch in iterator:
        for _, (sampled_batch, mim_mask) in enumerate(trainloader):
            ...
            unimg = torch.cat((unimg_a, unimg_b), dim=0)
            ...
            with torch.no_grad():
                ...
                plab_un = torch.cat((plab_a, plab_b), dim=0)
                plab_un = plab_un.unsqueeze(1)
            ...
            _, output_mim = model(unimg*(1-mim_mask))
            ...
            loss_mim = F.l1_loss(unimg, output_mim, reduction='none')
            loss_mim = (loss_mim * mim_mask).sum() / (mim_mask.sum() + 1e-5)

            loss = loss_l + loss_u + loss_mim
            ...
            loss.backward()
            optimizer.step()
            logging.info('iteration %d : loss_l: %03f, loss_u: %03f, loss_mim: %03f '%(iter_num, loss_l, loss_u, loss_mim))
...
if __name__ == "__main__":
    ## make logger file
    pre_snapshot_path = "./weight/Lung_{}_{}_labeled/pre_train".format(args.exp, args.labelnum)
    self_snapshot_path = "./weight/Lung_{}_{}_labeled/self_train".format(args.exp, args.labelnum)
