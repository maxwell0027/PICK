...
from dataloaders.dataset_pa import *
...
parser.add_argument('--max_samples', type=int,  default=60, help='maximum samples to train')
...
parser.add_argument('--lambda_', type=float, default=1.0, help='balance loss')
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, (sampled_batch, mim_mask) in enumerate(trainloader):
            ...
            if iter_num % 200 == 0:
                model.eval()
                val_dice, maxdice1, max_flag = test(model, model, test_loader, maxdice1, stride_xy=16, stride_z=4)
...
@torch.no_grad()
def test(net, ema_net, val_loader, maxdice=0, stride_xy=18, stride_z=4, main=True): # 18,4/ 16,4
    metrics = test_calculate_metric(net, ema_net, val_loader.dataset, main=main)
    val_dice = metrics[0]
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, (sampled_batch, mim_mask) in enumerate(trainloader):
            ...
            lalab = label_batch[:args.labeled_bs]
            ...
            with torch.no_grad():
                ...
                '''
                un_main_outputs = model(unimg)
                un_main_ps_lab = get_cut_mask(un_main_outputs, nms=1)
                un_pl = un_main_ps_lab.long()
                un_main_ps_lab = un_main_ps_lab.unsqueeze(1).detach().int()
            ...
            # forward of main decoder
            main_outputs = model(torch.cat((cutmix_batch_f, cutmix_batch_b), dim=0))
            ...
            # loss for the aux decoder
            loss_aux_ce = F.cross_entropy(aux_outputs, torch.cat((un_pl, lalab), dim=0))
            loss_aux_dice = DICE(aux_outputs, torch.cat((un_pl, lalab), dim=0))
            loss_aux = (loss_aux_ce + loss_aux_dice) / 2
...
if __name__ == "__main__":
    ## make logger file
    pre_snapshot_path = "./weight/PA_{}_{}_labeled_{}/pre_train".format(args.exp, args.labelnum, args.lambda_)
    self_snapshot_path = "./weight/PA_{}_{}_labeled_{}/self_train".format(args.exp, args.labelnum, args.lambda_)
