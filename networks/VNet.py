...
class Decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        x8_up = x8_up_ori + x1
        x9_ori = self.block_nine(x8_up)
        # x9 = F.dropout3d(x9, p=0.5, training=True)
        if self.has_dropout:
            x9 = self.dropout(x9_ori)
        out_seg = self.out_conv(x9_ori)
        return out_seg, x8
...
'''
class VNet_my(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        super(VNet_my, self).__init__()

        self.encoder = Encoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual)
        self.decoder = Decoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual)
        self.decoder_mim = Decoder(n_channels, 1, n_filters, normalization, has_dropout, has_residual)



    def forward(self, input):
        features = self.encoder(input)
        out_seg, _ = self.decoder(features)
        out_mim, _ = self.decoder_mim(features)

        return out_seg, out_mim
'''
